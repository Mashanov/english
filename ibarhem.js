...
const it = (i) => {
	...
	if (typeof i != 'object' && typeof i != 'string') return alert ('Ошибка в передаваемых данных', 'error');

/* Если в качестве объекта указан элемент */
	if (typeof i == 'object')
	{

		if (i != d) return alert ('Ошибка в передаваемых данных', 'error');

	} else {

		if (typeof i == 'string' && d.querySelector(i)) i = d.querySelectorAll(i);
		else return alert ('Ошибка в передаваемых данных', 'error');
	};
...
// Функция Добавления клика
	...
	/* Если передали обычный объект - Ловим клики с элемента*/
		else for (var i = 0; i < this.element.length; i++) this.element[i].addEventListener('click', fun);
	};
...
// Функция удаления элементов
	...
	* Удаляем их
	*/
		for (var i = 0; i < this.element.length; i++) this.element[i].remove();
	};
